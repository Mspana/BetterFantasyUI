...
/* The league photo runs above the masthead. It is shown whole rather than
   cropped -- it carries the league name and everyone's faces, and a banner
   crop would cut one or the other. */
.banner{background:var(--ink);display:flex;justify-content:center}
.banner img{display:block;width:100%;max-width:550px;height:auto}
@media (max-width:700px){.banner img{max-width:100%}}
header.top{border-bottom:1px solid var(--line);background:var(--surface);
  padding-block:16px 0;margin-bottom:22px}
/* The league name lives in the banner image now. The h1 stays in the markup
   but out of sight, so the page keeps a real heading for screen readers and
   search without printing the name twice. */
...
/* The portrait sits in the top-right corner of the masthead, big, with the
   team name under it. The header reserves room on the right so the league
   name never runs beneath it. */
.whoisit{display:flex;flex-direction:column;align-items:center;gap:7px;cursor:pointer;
  font:inherit;color:var(--ink);background:none;border:0;padding:0}
.whoisit img{width:clamp(84px,9.5vw,132px);aspect-ratio:1;border-radius:5px;
  object-fit:cover;display:block;background:var(--raised);
  border:1px solid var(--line);box-shadow:var(--shadow)}
.whoisit span{font-family:"IBM Plex Mono",monospace;font-size:10.5px;letter-spacing:.09em;
  text-transform:uppercase;color:var(--muted);max-width:clamp(84px,9.5vw,132px);
  text-align:center;line-height:1.35}
.whoisit:hover span{color:var(--ink)}
.whoisit:focus-visible{outline:2px solid var(--accent);outline-offset:3px;border-radius:5px}
@media (min-width:701px){
  header.top .wrap{position:relative;padding-right:clamp(108px,11.5vw,160px)}
...
/* Block, not flex. As a column flex container this shrank the grid whenever
   ...
   .pshot clips its images, so they vanished instead of scrolling. Whether it
   happened depended on viewport height, which is why it came and went. */
#picker{position:fixed;inset:0;z-index:60;background:var(--paper);
  overflow-y:auto;-webkit-overflow-scrolling:touch;
  padding-top:env(safe-area-inset-top,0px);padding-bottom:env(safe-area-inset-bottom,0px)}
#whoami{font-family:Oswald,sans-serif;font-weight:500;text-transform:uppercase;
  letter-spacing:.03em;text-align:center;margin:0;cursor:pointer;
  font-size:clamp(30px,7vw,64px);line-height:1.1;padding:18px 16px 14px;
  -webkit-user-select:none;user-select:none}
/* minmax(0,1fr), not 1fr: a bare 1fr floors each track at the item's
   min-content width, and the captions do not wrap -- so a long team name
   ("The Matt Fundraised Conference") made its column wider than the others
   and pushed the grid off the screen. */
.pgrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0;width:100%}
@media (max-width:1000px){.pgrid{grid-template-columns:repeat(3,minmax(0,1fr))}}
@media (max-width:700px){.pgrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
.ptile{display:block;width:100%;padding:0;border:0;background:none;cursor:pointer;
  font:inherit;color:inherit;text-align:center;touch-action:manipulation;
  -webkit-tap-highlight-color:transparent;-webkit-user-select:none;user-select:none}
.pshot img{-webkit-user-drag:none;user-drag:none;pointer-events:none}
/* both are spans inside a button: without display:block they stay inline,
   width and aspect-ratio do not apply, and the whole tile collapses */
.pshot{display:block;position:relative;width:100%;aspect-ratio:1;overflow:hidden;
  background:var(--raised)}
.pshot img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block}
.pshot .p2{opacity:0;transition:opacity .12s ease}
/* hover reveals the second photo; the heading flips them all, and hovering
   while flipped flips that one back -- the two toggles compose */
/* Hover rules only where a pointer can actually hover: on a touchscreen
   :hover sticks after a tap and the photo never changes back. */
@media (hover:hover){
  .ptile:hover .pshot .p2{opacity:1}
  .pgrid.flipped .ptile:hover .pshot .p2{opacity:0}
}
.ptile:focus-visible .pshot .p2{opacity:1}
.pgrid.flipped .pshot .p2{opacity:1}
.pgrid.flipped .ptile:focus-visible .pshot .p2{opacity:0}
/* Touch: held down shows the second photo, releasing picks the team. */
.ptile.pressing .pshot .p2{opacity:1}
.pgrid.flipped .ptile.pressing .pshot .p2{opacity:0}
.ptile:focus-visible{outline:3px solid var(--accent);outline-offset:-3px}
/* NOT .pname -- that class already labels every player name in the tables,
   and this rule comes later in the sheet, so it would restyle all of them */
/* Wrap rather than truncate: at phone width a track is under 200px and the
   longer team names would read "THE MATT FUNDRAISED CONFERENC...". */
.ptname{display:block;font-family:"IBM Plex Mono",monospace;font-size:11px;letter-spacing:.06em;
  text-transform:uppercase;color:var(--muted);padding:7px 8px 12px;line-height:1.35;
  overflow-wrap:anywhere}
.ptile:hover .ptname{color:var(--ink)}
@media (prefers-reduced-motion:reduce){.pshot .p2{transition:none}}
...
@media (max-width:560px){.row{font-size:12.5px}header.top{padding-block:16px 0}
  .teampick{margin-left:0;flex:1 1 100%;justify-content:flex-start}#team{flex:1}
  .whoisit{flex-direction:row;gap:10px}
  .whoisit img{width:56px}
  .whoisit span{max-width:none;text-align:left;font-size:11.5px}}
...
/* ---- who are you ----
   A first-run chooser that only sets "Viewing as". The choice is remembered
   per browser, so it is asked once and never again on that device. */
...
function showPicker(){
  const el = $("#picker");
  if (!el || !(D.people || []).length) return false;
  renderPicker();
  el.hidden = false;
  return true;
}
...
function choose(t){
  ...
  const sel = $("#team");
  if (sel) sel.value = t;
  TRADE.give.clear(); TRADE.recv.clear(); clearFound();
  renderAll();
}
...
/* Touch picking: press shows the other photo, release selects. A tap also
   ...
   twice; mouse and keyboard still go through click as normal. */
let swallowClick = false, pressed = null, pressAt = null;
const DRAG = 10;      // px of travel that turns a tap into a drag or a scroll
let dragged = false;
...
/* A drag past the threshold is a scroll or a swipe, not a pick. Without this
   the release lands on whatever tile is under the finger and chooses it. */
function pressMove(e){
  if (!pressed || !pressAt) return;
  if (Math.abs(e.clientX - pressAt.x) > DRAG || Math.abs(e.clientY - pressAt.y) > DRAG){
    dragged = true;
    clearPress();
  }
}
...
function pressPick(e){
  ...
  clearPress();
  if (!tile || tile !== started) return;      // released somewhere else: no pick
  if (e.pointerType === "touch" || e.pointerType === "pen"){
    swallowClick = true;
    choose(tile.dataset.team);
  }
}
...
on(document, "click", e=>{
  ...
  if (tile){
    ...
    if (e.detail && dragged){ dragged = false; return; }   // mouse drag, not a click
    dragged = false;
    choose(tile.dataset.team);
    return;
  }
...
on(document, "pointerdown", pressOn, "pdown");
on(document, "pointermove", pressMove, "pmove");
on(document, "pointerup", pressPick, "pup");
on(document, "pointercancel", clearPress, "pcancel");
on(window, "blur", clearPress, "pblur");
...
def build(data, src_path):
    ...
    # an artifact takes 255 files in total, so the portraits share the budget
    budget = MAX_PHOTOS - 2 * len(people)
    photos = {}
...
<div class="wrap">
  ...
  <div class="tablewrap" id="powerwrap">
    <table><thead><tr>
      <th class="r">#</th><th class="pface"></th><th>Team</th>
